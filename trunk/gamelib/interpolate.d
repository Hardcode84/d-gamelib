...
module gamelib.interpolate;

private {
    import gamelib.linalg : Vector, dot, vec2, vec3, vec4, quat;
    import gamelib.util : is_vector, is_quaternion;
    import gamelib.math : almost_equal, acos, sin, sqrt, clamp, PI;
    import std.conv : to;
}
...
auto interp_spherical(T1,T2)(in T1 a, in T1 b, in T2 t) if(is_vector!T1 || is_quaternion!T1) {
    static if(is_vector!T1) {
        const theta = acos(dot(a, b));
    } else {
        const theta = acos(
